...
class IDetect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=()):  # detection layer
        ...
        self.register_buffer('anchors', a)  # shape(nl,na,2)
        self.register_buffer('anchor_grid', a.clone().view(self.nl, 1, -1, 1, 1, 2))  # shape(nl,1,na,1,1,2)
        # self.dyhead = nn.Sequential(*[DyHeadBlock(ch[0]) for i in range(2)])  # dyhead的代码
    ...
    def forward(self, x):
        # for dyhead_layer in self.dyhead:  # dyhead的代码
        #     x = dyhead_layer(x)  # dyhead的代码
        # x = x.copy()  # for profiling
        z = []  # inference output
        self.training |= self.export
        for i in range(self.nl):
...
class IKeypoint(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), nkpt=17, ch=(), inplace=True, dw_conv_kpt=False):  # detection layer
        ...
        self.no_det=(nc + 5)  # number of outputs per anchor for box and class1
...
class Model(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, IDetect):
            s = 256  # 2x min stride
            m.stride = torch.tensor([s / x.shape[-2] for x in self.forward(torch.zeros(1, ch, s, s))])  # forward
            # m.stride = torch.tensor([s / x.shape[-2] for x in self.forward(torch.zeros(2, ch, s, s))])  # forward,ODConv卷积更改
            check_anchor_order(m)
            m.anchors /= m.stride.view(-1, 1, 1)
            self.stride = m.stride
            self._initialize_biases()  # only run once
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            ...
            if self.traced:
                if isinstance(m, Detect) or isinstance(m, IDetect) or isinstance(m, IDetect_Decoupled) or isinstance(m, IAuxDetect) or isinstance(m, IKeypoint) or isinstance(m, TSCODE_Detect):
                    break

            if profile:
                c = isinstance(m, (Detect, IDetect, IDetect_Decoupled, IAuxDetect, IBin, TSCODE_Detect))
    ...
    def _initialize_biases(self, cf=None):  # initialize biases into Detect(), cf is class1 frequency
    ...
    def _initialize_aux_biases(self, cf=None):  # initialize biases into Detect(), cf is class1 frequency
        # https://arxiv.org/abs/1708.02002 section 3.3
        # cf = torch.bincount(torch.tensor(np.concatenate(dataset.labels, 0)[:, 0]).long(), minlength=nc) + 1.
        m = self.model[-1]  # Detect() module
    ...
    def _initialize_biases_bin(self, cf=None):  # initialize biases into Detect(), cf is class1 frequency
        # https://arxiv.org/abs/1708.02002 section 3.3
        # cf = torch.bincount(torch.tensor(np.concatenate(dataset.labels, 0)[:, 0]).long(), minlength=nc) + 1.
    ...
    def _initialize_biases_kpt(self, cf=None):  # initialize biases into Detect(), cf is class1 frequency
        # https://arxiv.org/abs/1708.02002 section 3.3
        # cf = torch.bincount(torch.tensor(np.concatenate(dataset.labels, 0)[:, 0]).long(), minlength=nc) + 1.
        m = self.model[-1]  # Detect() module
        for mi, s in zip(m.m, m.stride):  # from
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC, 
                 ...
                 Yolov7_Tiny_E_ELAN, Yolov7_Tiny_SPP, Yolov7_E_ELAN, Yolov7_E_ELAN_NECK, Yolov7_E_ELAN_PConv,
                 Yolov7x_E_ELAN,
                 DCNv2, SAConv2d, CoordConv, DSConv2D, PConv, Conv_ATT, ODConv2d, RFEM, GSConv, VoVGSCSP, VoVGSCSPC, SPPCSPC_ATT]:
            c1, c2 = ch[f], args[0]
            ...
            c2 = ch[f] // 2
        elif m in [Detect, TSCODE_Detect, IDetect_Decoupled, IDetect, IAuxDetect, IBin, IKeypoint]:
...
if __name__ == '__main__':
    ...
    if opt.profile:
        # img = torch.rand(2, 3, 640, 640).to(device)  # ODConv更改
        img = torch.rand(1, 3, 640, 640).to(device)
