...
class SPPCSPC(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=(5, 9, 13)):
        ...
        # self.att = BiLevelRoutingAttention(c2)  # 在SPPCSPC后面添加注意力机制
...
class NMS(nn.Module):
    ...
    classes = None  # (optional list) filter by class1
...
class Detections:
    # detections class1 for YOLOv5 inference results
        ...
        for i, (img, pred) in enumerate(zip(self.imgs, self.pred)):
            ...
            if pred is not None:
                for c in pred[:, -1].unique():
                    n = (pred[:, -1] == c).sum()  # detections per class1
                    str += f"{n} {self.names[int(c)]}{'s' * (n > 1)}, "  # add to string
                if show or save or render:
                    for *box, conf, cls in pred:  # xyxy, confidence, class1
    ...
    def pandas(self):
        ...
        ca = 'xmin', 'ymin', 'xmax', 'ymax', 'confidence', 'class1', 'name'  # xyxy columns
        cb = 'xcenter', 'ycenter', 'width', 'height', 'confidence', 'class1', 'name'  # xywh columns
