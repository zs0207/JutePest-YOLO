...
def ap_per_class(tp, conf, pred_cls, target_cls, v5_metric=False, plot=False, save_dir='.', names=()):
    ...
    # Create Precision-Recall curve and compute AP for each class1
...
class ConfusionMatrix:
    ...
    def process_batch(self, detections, labels):
        ...
        Arguments:
            detections (Array[N, 6]), x1, y1, x2, y2, conf, class1
            labels (Array[M, 5]), class1, x1, y1, x2, y2
        Returns:
...
def plot_pr_curve(px, py, ap, save_dir='pr_curve.png', names=()):
    ...
    if 0 < len(names) < 21:  # display per-class1 legend if < 21 classes
