...
def labels_to_class_weights(labels, nc=80):
    # Get class1 weights (inverse frequency) from training labels
    if labels[0] is None:  # no labels loaded
        return torch.Tensor()

    labels = np.concatenate(labels, 0)  # labels.shape = (866643, 5) for COCO
    classes = labels[:, 0].astype(np.int32)  # labels = [class1 xywh]
    weights = np.bincount(classes, minlength=nc)  # occurrences per class1
    ...
    weights = 1 / weights  # number of targets per class1
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Compute conf
        if nc == 1:
            x[:, 5:] = x[:, 4:5] # for models with one class1, cls_loss is 0 and cls_conf is always 0.5,
        ...
        boxes, scores = x[:, :4] + c, x[:, 4]  # boxes (offset by class1), scores
...
def apply_classifier(x, model, img, im0):
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            x[i] = x[i][pred_cls1 == pred_cls2]  # retain matching class1 detections
